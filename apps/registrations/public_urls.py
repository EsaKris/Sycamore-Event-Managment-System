...
urlpatterns = [
    # Literal paths first — 'check-returning', 'worker' and '' would
    # otherwise be swallowed by the <slug:event_slug> catch-all below,
    # since a bare segment is a syntactically valid slug.
    path('', public_views.public_register_default, name='register_default'),
    path('worker/', public_views.public_register_worker_default, name='register_worker_default'),
